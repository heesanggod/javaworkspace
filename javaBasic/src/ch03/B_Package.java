...
public class B_Package {
	...
	public static void main(String[] args) {
		...
		// (많이사용)*ArreyList: List 중에서 내부 배열을 사용하여 인덱스를 사용한 검색이 빠름
		...
		arrayList.add(10);   // 배열을 추가하는 것
		System.out.println(arrayList);
		// arrayList.add("aaa");    // List<Object> 또는 <>를 지움
		
		// get(): List 의 특정 인덱스 요소에 접근
		// List 인덱스 범위를 초과하는 인덱스를 지정할 수 없음
		// 기본형 데이터타입(int)의 변수로 반환값을 받을 경우 null 값을 받을 수 있음을 주의   //  참조형타입(Integer)
		...
		// Set: 데이터를 저장할 때 중복을 혀용하지 않고 순서가 존재하지 않음 (집합)
		Set<String> set;
		// HashSet: 해싱 기법을 사용해서 검색 속도가 빠름
		// set = new HashSet<>();
		// TreeSet: 요소를 정렬해서 추가하는 
		set = new TreeSet<>();
		...
		System.out.println(set.contains("Banana"));  // 존재: true 아니면 false
